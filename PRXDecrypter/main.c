...
int pspDecompress(const u8 *inbuf, u8 *outbuf, u32 outcapacity) {
	if (kernelver == 0x01050001) {
		...
		return pspDecompress_02g(inbuf, outbuf, outcapacity);
}
...
int sceUtilsBufferCopyWithRange_(void *inbuf, SceSize insize, void *outbuf, int outsize, int cmd) {
	if (kernelver == 0x01050001) {
		...
		return sceUtilsBufferCopyWithRange_02g(inbuf, insize, outbuf, outsize, cmd);
}
...
int kirk7(u8* prx, u32 size, u32 scramble_code, u32 use_polling) {
	...
	if (!use_polling) {
		ret = sceUtilsBufferCopyWithRange_(prx, size + 20, prx, size + 20, 7);
	} else {
		ret = /*sceUtilsBufferCopyByPollingWithRange*/sceUtilsBufferCopyWithRange_(prx, size + 20, prx, size + 20, 7);
	}
...
int _kprx_decrypt(u8 *prx, u32 size, u32 *newsize, u32 use_polling) {
	...
	*newsize = *(u32*) &prx[0xb0];
	...
	if (!use_polling) {
		ret = sceUtilsBufferCopyWithRange_(buf_1, 0x150, buf_1, 0x150, 0xB);
	} else {
		ret = /*sceUtilsBufferCopyByPollingWithRange*/sceUtilsBufferCopyWithRange_(buf_1, 0x150, buf_1, 0x150, 0xB);
	}
	...
	if (!use_polling) {
		ret = sceUtilsBufferCopyWithRange_(prx, size, prx + 0x40, size - 0x40, 1);
	} else {
		ret = /*sceUtilsBufferCopyByPollingWithRange*/sceUtilsBufferCopyWithRange_(prx, size, prx + 0x40, size - 0x40, 1);
	}
	...
	ret = 0;
	exit: memset(g_kernel_phat_key, 0, 0x10);
	memset(g_kernel_slim_key, 0, 0x10);
...
// sigcheck keys
u8 check_keys0[0x10] = { 0x71, 0xF6, 0xA8, 0x31, 0x1E, 0xE0, 0xFF, 0x1E, 0x50, 0xBA, 0x6C, 0xD2, 0x98, 0x2D, 0xD6, 0x2D };

u8 check_keys1[0x10] = { 0xAA, 0x85, 0x4D, 0xB0, 0xFF, 0xCA, 0x47, 0xEB, 0x38, 0x7F, 0xD7, 0xE4, 0x3D, 0x62, 0xB0, 0x10 };
...
void SaveOutput(char *fileout, u8 *psave, int savesize) {
	...
	if (!SaveFile(fileout, psave, savesize))
		printfLogged("save failed.");
	else
		printfLogged("saved.");
		...
		for (key = 0; key <= 0xFF; key += 0x01) {
			...
			for (i = 0; i < savesize; i += 4) {
				...
				if ((((*(u32 *) (psave + i)) ^ xorkey) == 0x5053507E) || ((!key) && (*(u32*) (psave + i) == 0x11724A68))) {
					...
					if (!key)
						printfLogged("--> %s @ 0x%08X .. ", modname, i);
					else
						printfLogged("--> %s @ 0x%08X/XOR 0x%02X .. ", modname, i, key);
					...
					if (isSpecialPrx(*(u32*) (psave + i))) {
						...
						modsize = ((*(u32 *) (psave + i + 0x2C)) ^ xorkey);
					...
					if (!SaveFile(fileoutpath, psave + i, modsize))
						printfLogged("failed to save\n");
					else
						printfLogged("saved (%i bytes)\n", modsize);
...
void AnalyzeSinglePRX(char *filein) {
	...
	if (size > 0) {
		...
		if (!memcmp(g_dataOut + 1, "ELF", 3)) {
			...
			if (!tag_found) {
				if (IsSignChecked(g_dataOut)) {
					...
					if (!tag_found) {
						...
						printfLogged("known tag 0x%08X, ", tag);
				} else {
			...
			if ((!memcmp(g_dataOut + 0xA, "updater", 7)) || (!memcmp(g_dataOut + 0xA, "launcher", 7)))
				printfLogged("contains embedded modules, ");
		} else {
...
void errorexit(int errorcode) {
	...
	switch (errorcode) {
	case 1:
		printf("%s folder is missing", DEST);
		break;
	case 2:
		printf("Unable to initialize kernel functions");
		break;
	case 3:
		printf("Failed to get list of files from %s", DEST);
		break;
	case 4:
...
int main(void) {
	...
	char sysmem270path[strlen(DEST) + 16];	//sysmem_rlz.prx
	...
	if (kernelver == 0x01050001) {
		...
		if (kmod < 0) {
			...
			printfLogged("Successfully loaded PRXdecrypter01g module\n");
	} else {
		...
		if (kmod < 0) {
			...
			printfLogged("Successfully loaded PRXdecrypter02g module\n\n");
	...
	restartapp: printfLogged("* Going to main menu...");
	...
	if (notdecrypted && notunsignchecked)
		pspDebugScreenSetTextColor(COLOR_WHITE);
	printf("   Unsigncheck files\n");
	y++;
	pspDebugScreenSetTextColor(COLOR_GREY);

	if (notdecrypted && notresignchecked)
		pspDebugScreenSetTextColor(COLOR_WHITE);
	printf("   Signcheck files\n");
	y++;
	pspDebugScreenSetTextColor(COLOR_GREY);

	if (rebootfiles && rebootnotextracted)
		pspDebugScreenSetTextColor(COLOR_WHITE);
	printf("   Extract reboot.bin\n");
	y++;
	pspDebugScreenSetTextColor(COLOR_GREY);

	if (rebootfiles_02g && reboot_02g_notextracted)
		pspDebugScreenSetTextColor(COLOR_WHITE);
	...
	rowprev = 0;
	maxrows = (y - 1);
	if ((row == 0) && (!notdecrypted))
		row++;
	if ((row == 2) && (!notunsignchecked || !notdecrypted))
		row++;
	if ((row == 3) && (!notresignchecked || !notdecrypted))
		row++;
	if ((row == 4) && (!rebootfiles || !rebootnotextracted))
		row++;
	if ((row == 5) && (!rebootfiles_02g || !reboot_02g_notextracted))
		row++;
	refreshArrow();
	...
	if (!(fileGetList(inpath, NULL, FILE_LIST_FILE | FILE_LIST_RECURSIVE, &filelist))) {
		...
		while (z < filelist.count) {
			...
			if (!strcmp(friendlyname2, "log.txt"))
				goto nextfile;
			...
			if (analyze_only)
				AnalyzeSinglePRX(filelist.list[z]);
			else
				DecryptSinglePRX(filelist.list[z], savepath);
			printfLogged("\n");
		...
		if (updatermods) {
			...
			if (b & PSP_CTRL_TRIANGLE) {
				...
				if (!strcmp(outpath, "ms0:/dec/")) {
					...
					if (checkfolder < 0)
						sceIoMkdir("ms0:/dec/updaterprx", 0777);
					else
						sceIoDclose(checkfolder);
