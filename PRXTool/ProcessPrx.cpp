...
static const char* g_szRelTypes[16] = { "R_NONE", "R_16", "R_32", "R_REL32", "R_26", "R_HI16", "R_LO16", "R_GPREL16", "R_LITERAL",
...
CProcessPrx::CProcessPrx(u32 dwBase) :
		CProcessElf(), m_defNidMgr(), m_pCurrNidMgr(&m_defNidMgr), m_pElfRelocs(NULL), m_iRelocCount(0), m_dwBase(dwBase), m_stubBottom(0), m_blXmlDump(
				false) {
		...
		do {
			...
			for (iLoop = 0; iLoop < pLib->v_count; iLoop++) {
				...
				COutput::Printf(LEVEL_DEBUG, "Found variable nid:0x%08X addr:0x%08X name:%s\n", pLib->vars[iLoop].nid,
						pLib->vars[iLoop].addr, pLib->vars[iLoop].name);
...
int CProcessPrx::CountRelocs() {
	...
	for (iLoop = 0; iLoop < m_iPHCount; iLoop++) {
		if (m_pElfPrograms[iLoop].iType == PT_PRXRELOC2) {
			...
			if (m_pElfPrograms[iLoop].pData[0] != 0 || m_pElfPrograms[iLoop].pData[1] != 0) {
			...
			if (COutput::GetDebug()) {
				...
				for (iLoop = 0; iLoop < m_iRelocCount; iLoop++) {
					if (m_pElfRelocs[iLoop].type < 16) {
						COutput::Printf(LEVEL_DEBUG, "Reloc %s:%d Type:%s Symbol:%d Offset %08X Info:%08X\n", m_pElfRelocs[iLoop].secname,
								iLoop, g_szRelTypes[m_pElfRelocs[iLoop].type], m_pElfRelocs[iLoop].symbol, m_pElfRelocs[iLoop].offset,
								m_pElfRelocs[iLoop].info);
					} else {
						COutput::Printf(LEVEL_DEBUG, "Reloc %s:%d Type:%d Symbol:%d Offset %08X\n", m_pElfRelocs[iLoop].secname, iLoop,
								m_pElfRelocs[iLoop].type, m_pElfRelocs[iLoop].symbol, m_pElfRelocs[iLoop].offset);
					}
				}
			}
		}
	...
	for (iLoop = 0; iLoop < m_iRelocCount; iLoop++) {
		...
		case R_MIPS_HI16: {
			...
			while (++iLoop < m_iRelocCount) {
				if (m_pElfRelocs[iLoop].type != R_MIPS_HI16)
					break;
			}
			...
			while (iLoop < m_iRelocCount) {
				...
				if (m_pElfRelocs[++iLoop].type != R_MIPS_LO16)
					break;
			}
			iLoop--;
		...
		if (m_pElfSections[iLoop].iFlags & (SHF_EXECINSTR | SHF_ALLOC)) {
			if ((m_pElfSections[iLoop].iSize > 0) && (m_pElfSections[iLoop].iType == SHT_PROGBITS)) {
				fprintf(fp, "\n; ==== Section %s - Address 0x%08X Size 0x%08X Flags 0x%04X\n", m_pElfSections[iLoop].szName,
						m_pElfSections[iLoop].iAddr + m_dwBase, m_pElfSections[iLoop].iSize, m_pElfSections[iLoop].iFlags);
				if (m_pElfSections[iLoop].iFlags & SHF_EXECINSTR) {
					Disasm(fp, m_pElfSections[iLoop].iAddr + m_dwBase, m_pElfSections[iLoop].iSize,
							(u8*) m_vMem.GetPtr(m_pElfSections[iLoop].iAddr), m_imms, m_dwBase);
				} else {
					DumpData(fp, m_pElfSections[iLoop].iAddr + m_dwBase, m_pElfSections[iLoop].iSize,
							(u8*) m_vMem.GetPtr(m_pElfSections[iLoop].iAddr));
					DumpStrings(fp, m_pElfSections[iLoop].iAddr + m_dwBase, m_pElfSections[iLoop].iSize,
							(u8*) m_vMem.GetPtr(m_pElfSections[iLoop].iAddr));
				}
			}
		}
	...
	for (iLoop = 0; iLoop < m_iSHCount; iLoop++) {
		if (m_pElfSections[iLoop].iFlags & (SHF_EXECINSTR | SHF_ALLOC)) {
			if ((m_pElfSections[iLoop].iSize > 0) && (m_pElfSections[iLoop].iType == SHT_PROGBITS)) {
				if (m_pElfSections[iLoop].iFlags & SHF_EXECINSTR) {
					fprintf(fp, "<disasm>\n");
					DisasmXML(fp, m_pElfSections[iLoop].iAddr + m_dwBase, m_pElfSections[iLoop].iSize,
							(u8*) m_vMem.GetPtr(m_pElfSections[iLoop].iAddr), m_imms, m_dwBase);
					fprintf(fp, "</disasm>\n");
				}
			}
