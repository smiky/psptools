...
int do_serialize(const char *arg) {
	...
	while (arg[i]) {
		...
		case 'i':
			g_iSMask |= SERIALIZE_IMPORTS;
			break;
		case 'x':
			g_iSMask |= SERIALIZE_EXPORTS;
			break;
		case 'r':
			g_iSMask |= SERIALIZE_RELOCS;
			break;
		case 's':
			g_iSMask |= SERIALIZE_SECTIONS;
			break;
		case 'l':
			g_iSMask |= SERIALIZE_DOSYSLIB;
			break;
		default:
			COutput::Printf(LEVEL_WARNING, "Unknown serialize option '%c'\n", tolower(arg[i]));
			return 0;
		};
		i++;
	}
	...
	if (prx.LoadFromFile(file) == false) {
		...
		while (pExport != NULL) {
			COutput::Printf(LEVEL_INFO, "Export %d, Name %s, Functions %d, Variables %d, flags %08X\n", count++, pExport->name,
					pExport->f_count, pExport->v_count, pExport->stub.flags);
		...
		while (pImport != NULL) {
			COutput::Printf(LEVEL_INFO, "Import %d, Name %s, Functions %d, Variables %d, flags %08X\n", count++, pImport->name,
					pImport->f_count, pImport->v_count, pImport->stub.flags);
		...
		while (pExport != NULL) {
			...
			if (pExport->f_count > 0) {
				...
				for (iLoop = 0; iLoop < pExport->f_count; iLoop++) {
					...
					COutput::Printf(LEVEL_INFO, "0x%08X [0x%08X] - %s", pExport->funcs[iLoop].nid, pExport->funcs[iLoop].addr,
							pExport->funcs[iLoop].name);
					if (g_aliasOutput) {
						SymbolEntry *pSym;
			...
			if (pExport->v_count > 0) {
				...
				for (iLoop = 0; iLoop < pExport->v_count; iLoop++) {
					COutput::Printf(LEVEL_INFO, "0x%08X [0x%08X] - %s\n", pExport->vars[iLoop].nid, pExport->vars[iLoop].addr,
							pExport->vars[iLoop].name);
				}
			}
...
int main(int argc, char **argv) {
	...
	if (process_args(argc, argv)) {
		...
		switch (g_outputMode) {
		case OUTPUT_XML:
			pSer = new CSerializePrxToXml(out_fp);
			break;
		case OUTPUT_MAP:
			pSer = new CSerializePrxToMap(out_fp);
			break;
		case OUTPUT_IDC:
			pSer = new CSerializePrxToIdc(out_fp);
			break;
		default:
			pSer = NULL;
			break;
